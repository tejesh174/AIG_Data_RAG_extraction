...
- Negative values shown in parentheses should be returned as negative number.
- Return ONLY valid JSON with no explanation or markdown."""


#Building the extraction prompt based on variable type
def make_user_prompt(company: str, year: int, variable: dict, context: str) -> str:
    ...
    if variable["name"] == "total_revenues":
        target = (
            f"Return the value of '{variable['display']}' for fiscal year {year}."
        )

    elif variable["name"] == "net_income":
        target = (
            f"Find 'Net income (loss) attributable to {company}' "
            f"and return the value for fiscal year {year}."
)
    else:
        target = (
            "Identify the company's primary/core reportable business segments "
            "as described in the filing. Return them as a single string"
        )
    ...
    if var_type == "numeric":
        instructions = (
            ...
            f"Unit: {unit}.\n"
            f"{target}\n\n"
            "Return the value as a number only, in millions, with no commas or currency symbols.\n"
            "If the table shows parentheses like (6,084), return a negative number like -6084.\n"
            "If the value is not present in the excerpts, return null.\n\n"
        ...
        instructions = (
            ...
            f"{target}\n\n"
            'Return JSON: {"extracted_value": "General Insurance and Life and Retirement"}'
        )
